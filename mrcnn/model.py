...
def detection_targets_graph(proposals, gt_class_ids, gt_boxes, gt_masks, gt_poses, config):
    ...
    gt_poses: [MAX_GT_INSTANCES, (rx, ry, rz, tz)]
    ...
    poses: [TRAIN_ROIS_PER_IMAGE, NUM_CLASSES, (rx, ry, rz, tz)]
    ...
    gt_poses, _ = trim_zeros_graph(gt_poses, name="trim_gt_poses")
    # Handle COCO crowds
    ...
    poses = tf.pad(roi_gt_poses, [(0, N + P), (0, 0)])
...
class DetectionTargetLayer(KE.Layer):
    ...
    gt_poses: [batch, MAX_GT_INSTANCES, (rx, ry, rz, tz)] unnormalized.
    Returns: Target ROIs and corresponding class IDs, bounding box shifts,
    and masks.
    ...
    def compute_output_shape(self, input_shape):
        return [
            ...
            (None, self.config.TRAIN_ROIS_PER_IMAGE, self.config.MASK_SHAPE[0], self.config.MASK_SHAPE[1]),  # masks
             (None, self.config.TRAIN_ROIS_PER_IMAGE, 4) # poses
        ]
...
def fpn_classifier_graph(rois, feature_maps, image_meta,
    ...
    # Pose head
    # [batch, boxes, num_classes * (dy, dx, log(dh), log(dw))]
    #mrcnn_pose = KL.TimeDistributed(KL.Dense(num_classes * 4, activation='linear'),
    #                       name='mrcnn_pose_fc')(shared)
    ## Reshape to [batch, boxes, num_classes, (dy, dx, log(dh), log(dw))]
    #s_pose = K.int_shape(mrcnn_pose)
    #mrcnn_pose = KL.Reshape((s_pose[1], num_classes, 4), name="mrcnn_pose")(mrcnn_pose)    
...
def fpn_pose_graph(rois, feature_maps, image_meta,
                         pool_size, num_classes, train_bn=True):
    """Builds the computation graph of the feature pyramid network classifier
    and regressor heads.
    ...
    x = KL.TimeDistributed(BatchNorm(), name='mrcnn_pose_bn1')(x, training=train_bn)
    x = KL.Activation('relu')(x)
    x = KL.TimeDistributed(KL.Conv2D(1024, (1, 1)),
                           name="mrcnn_pose_conv2")(x)
    x = KL.TimeDistributed(BatchNorm(), name='mrcnn_pose_bn2')(x, training=train_bn)
    x = KL.Activation('relu')(x)
    x = KL.TimeDistributed(KL.Conv2D(384, (1, 1)),
                           name="mrcnn_pose_conv3")(x)
    x = KL.TimeDistributed(BatchNorm(), name='mrcnn_pose_bn3')(x, training=train_bn)
    x = KL.Activation('relu')(x)

    shared = KL.Lambda(lambda x: K.squeeze(K.squeeze(x, 3), 2),
                       name="pool_squeeze_pose")(x)
    # Pose head
    ...
    s = K.int_shape(x)
    mrcnn_pose = KL.Reshape((s[1], num_classes, 4), name="mrcnn_pose")(x)
...
def mrcnn_pose_loss_graph(target_pose, target_class_ids, pred_pose):
    """Loss for Mask R-CNN pose prediction.

    target_bbox: [batch, num_rois, (rx, ry, rz, tz)]
    target_class_ids: [batch, num_rois]. Integer class IDs.
    pred_bbox: [batch, num_rois, num_classes, (rx, ry, rz, tz)]
    """
    # Reshape to merge batch and roi dimensions for simplicity.
    target_class_ids = K.reshape(target_class_ids, (-1,))
    target_pose = K.reshape(target_pose, (-1, 4))
    pred_pose = K.reshape(pred_pose, (-1, K.int_shape(pred_pose)[2], 4))
...
def load_image_gt(dataset, config, image_id, augment=False, augmentation=None,
    ...
    pose: [instance_count, (rx, ry, rz, tz)]
    ...
    pose, _ = dataset.load_pose(image_id)
...
def data_generator(dataset, config, shuffle=True, augment=False, augmentation=None,
    ...
    - gt_poses: [batch, MAX_GT_INSTANCES, (rx, ry, rz, tz)]
        ...
        try:
            ...
            if b >= batch_size:
                inputs = [batch_images, batch_image_meta, batch_rpn_match, batch_rpn_bbox,
                          batch_gt_class_ids, batch_gt_boxes, batch_gt_masks, batch_gt_poses]
                outputs = []
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        if mode == "training":
            ...
            # Detection GT (class IDs, bounding boxes, masks, and 3D pose)
            ...
            if config.USE_MINI_MASK:
                ...
                input_gt_masks = KL.Input(
                    ...
                    name="input_gt_masks", dtype=bool)
            #4. GT 3D Pose
            input_gt_poses = KL.Input(shape=[None, 4], name="input_gt_poses", dtype=tf.float32)
        elif mode == "inference":
        ...
        if mode == "training":
            ...
            mrcnn_pose = fpn_pose_graph(rois, mrcnn_feature_maps, input_image_meta,
                                     config.POOL_SIZE, config.NUM_CLASSES,
                                     train_bn=config.TRAIN_BN)
            ...
            pose_loss = KL.Lambda(lambda x: mrcnn_pose_loss_graph(*x), name="mrcnn_pose_loss")([target_pose, target_class_ids, mrcnn_pose])

            # Model
            inputs = [input_image, input_image_meta,
                      input_rpn_match, input_rpn_bbox, input_gt_class_ids, input_gt_boxes, input_gt_masks, input_gt_poses]
            if not config.USE_RPN_ROIS:
                inputs.append(input_rois)
            outputs = [rpn_class_logits, rpn_class, rpn_bbox,
                       mrcnn_class_logits, mrcnn_class, mrcnn_bbox, mrcnn_mask, mrcnn_pose,
                       rpn_rois, output_rois,
                       rpn_class_loss, rpn_bbox_loss, class_loss, bbox_loss, mask_loss, pose_loss]
            ...
            mrcnn_mask = build_fpn_mask_graph(detection_boxes, mrcnn_feature_maps,
                                              ...
                                              config.NUM_CLASSES,
                                              train_bn=config.TRAIN_BN)
            mrcnn_pose = fpn_pose_graph(detection_boxes, mrcnn_feature_maps, input_image_meta,
                                     config.POOL_SIZE, config.NUM_CLASSES,
                                     train_bn=config.TRAIN_BN)
            model = KM.Model([input_image, input_image_meta, input_anchors],
                             [detections, mrcnn_class, mrcnn_bbox,
                                 mrcnn_mask, mrcnn_pose, rpn_rois, rpn_class, rpn_bbox],
                             name='mask_rcnn')
    ...
    def compile(self, learning_rate, momentum):
        ...
        loss_names = [
            ...
            "mrcnn_class_loss", "mrcnn_bbox_loss", "mrcnn_mask_loss", "mrcnn_pose_loss"]
    ...
    def unmold_detections(self, detections, mrcnn_mask, mrcnn_pose, original_image_shape,
                          image_shape, window):
    ...
    def detect(self, images, verbose=0):
        ...
        detections, _, _, mrcnn_mask, mrcnn_pose, _, _, _ =\
        ...
        for i, image in enumerate(images):
            final_rois, final_class_ids, final_scores, final_masks, final_poses =\
                self.unmold_detections(detections[i], mrcnn_mask[i], mrcnn_pose[i],
